...
function createQuestion(req, res) {
  ...
  db.run('INSERT INTO questions (quiz_id, text, category_id) VALUES (?, ?, ?)', [quiz_id, text, category_id], function (err) {
    if (err) {
      console.error('DB error in createQuestion:', err.message);
      return res.status(500).json({ error: 'Database error', details: err.message });
    }
    res.json({ success: true, id: this.lastID, quiz_id, text, category_id });
  });
}
